...
namespace Manage.API.Controllers
{
    ...
    public class BankController : ControllerBase
    {
        ...
        {
            BaseResponse response = await _bankService.GetAll(request);
            return Ok(response);
        }
        [HttpGet("{id}")]
        public async Task<IActionResult> GetAll(int id)
        {
            BaseResponse response = await _bankService.GetById(id);
            return Ok(response);
        }
        [HttpPut("bank-update")]
        public async Task<IActionResult> Update(UpdateBankDTO update)
        {
            BaseResponse response = await _bankService.Update(update);
            return Ok(response);
        }
        [HttpDelete("bank-delete")]
        public async Task<IActionResult> Delete(List<int> ids)
        {
            BaseResponse response = await _bankService.Delete(ids);
            return Ok(response);
        }
    }
}
