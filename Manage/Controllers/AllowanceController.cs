...
namespace Manage.API.Controllers
{
    ...
    public class AllowanceController : ControllerBase
    {
        ...
        {
            BaseResponse response = await _allowanceService.AddNew(allowanceDto);
            return Ok(response);
        }
        [HttpPost("allowance-get-all")]
        public async Task<IActionResult> GetAll( BaseRequest request)
        {
            BaseResponse response = await _allowanceService.GetAll(request);
            return Ok(response);
        }
        [HttpGet("{id}")]
        public async Task<IActionResult> GetAll(int id)
        {
            BaseResponse response = await _allowanceService.GetById(id);
            return Ok(response);
        }
        [HttpPut("allowance-update")]
        public async Task<BaseResponse> Update( UpdateAllowanceDTO update)
        {
            BaseResponse response = await _allowanceService.Update(update);
            return response;
        }
        [HttpDelete("allowance-delete")]
        public async Task<IActionResult> Delete(List<int> ids)
        {
            BaseResponse response = await _allowanceService.Delete(ids);
            return Ok(response);
        }
    }
}
